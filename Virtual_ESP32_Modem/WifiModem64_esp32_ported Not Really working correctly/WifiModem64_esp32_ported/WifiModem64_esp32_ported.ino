 ...
 * mostly working and tested, what is not working
...
#define LED_PIN 26 // Default Status LED
#define DCD_PIN 14          // DCD Carrier Status
#define CTS_PIN 16          // CTS Clear to Send, connect to host's RTS pin
#define RTS_PIN 17          // RTS Request to Send, connect to host's CTS pin
#define PIXEL_PIN 15        // Optional RGB Status LED
...
display.setTextAlignment(TEXT_ALIGN_LEFT);
  ...
  display.drawString(0,12,status.substring(0,9));

//  display.drawLine(0, 17, display.width()-1, 17, WHITE);

  //display.setCursor(0, 20);
  display.drawString(0,20,String(bauds[serialspeed]));
