...
#include "src/RH/RH_RF95.h"

HardwareSerial ComSerial(2);
RH_RF95<HardwareSerial> driver(&COMSerial);

#include "src/RH/RHMesh.h"
...
void setup() {
  M5.begin();
  ComSerial.begin(57600, SERIAL_8N1, 33, 32);
    Serial.begin(9600);
    if (!manager.init()) {
...
void loop() {
    ...
    if (addr > '0' && addr <= '4') {
        ...
        if (manager.sendtoWait(data, sizeof(data), addr - '0') == RH_ROUTER_ERROR_NONE) {
            ...
            Serial.println("sendtoWait failed. Are the intermediate meshes running?");
        }
