...
//for (int i=53; i< 163 ; i++) {
  //int temp = map(capture[i], 60, 240, 60,125);
  //capture[i] = temp;

  //if (capture[i] <= 74) {capture[i] = 50;}

  //drawPixel works, but isn't great
  //gfx_rca.drawPixel( i, oldCapture[i] * 1.19, BLACK);
  //gfx_rca.drawPixel( i, capture[i] * 1.19 , WHITE);
  
   //drawFastVLine doesn't work
   //gfx_rca.drawFastVLine(i, 20, oldCapture[i], BLACK);
   //gfx_rca.drawFastVLine(i, 20, capture[i], WHITE);
  
  gfx_rca.drawLine(i, oldCapture[i+1], i, oldCapture[i], BLACK);
  gfx_rca.drawLine(i, capture[i+1], i, capture[i], GREEN);

  //fillRect doesn't work
  //gfx_rca.fillRect(i, 20, 2, oldCapture[i], WHITE);
  //gfx_rca.fillRect(i, 20, 2, capture[i], BLACK);
