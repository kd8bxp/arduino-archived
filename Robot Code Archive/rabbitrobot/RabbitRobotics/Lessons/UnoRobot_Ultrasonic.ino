...
const int US_TRIGGER_PIN = 4; //The digital pins that the ultrasonic sensor is connected to on the UNO
const int US_RECEIVE_PIN = 3;
...
void loop(){ /*----( LOOP: RUNS CONSTANTLY )----*/
  ...
  if(echoDuration > DURATION_PER_INCH){ //If the object is over an inch away:
    ...
    analogWrite(R_ENABLE_PIN, 155); //Run the right motor at 60%    
  } 
