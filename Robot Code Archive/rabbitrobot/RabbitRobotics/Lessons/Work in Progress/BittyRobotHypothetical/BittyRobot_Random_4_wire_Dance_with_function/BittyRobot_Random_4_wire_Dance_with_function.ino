 ...
 * Aug 2, 2017 - Moved motor control to sub routines. LeRoy Miller, 2017
 * Routines are:
 * left(speed, duration)
...
void loop() { /*----( LOOP: RUNS CONSTANTLY )----*/
  ...
  int duration = random(250, 500); //Generate a random duration of time in milliseconds for the action to run for from 500ms to 5000ms
  if (action == SPIN_LEFT) { //Code block for spinning left
    left(driveSpeed, duration);
  } 
  else if (action == SPIN_RIGHT)
        { //Code block for spinning right
          right(driveSpeed, duration);
         }
