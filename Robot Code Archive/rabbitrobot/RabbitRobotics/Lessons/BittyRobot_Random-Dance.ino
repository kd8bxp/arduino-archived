...
int driveSpeed = 155; //Motor speed for the robot; can range from 255 (max power) to 0 (stop).
...
void loop() { /*----( LOOP: RUNS CONSTANTLY )----*/
  ...
  if (action == SPIN_LEFT) { //Code block for spinning left
   ...
   driveSpeed = constrain(driveSpeed, 0, 255); //Set driveSpeed into the allowable range of speeds if it goes over or under
