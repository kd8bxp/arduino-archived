...
Public Class Form1
    ...
    Dim RingFound As Boolean
    Dim RingTimer As Integer
    ...
    Private Sub UpdateStatusStripCom()
        ...
        ' read the serial port pins
        If SerialPort.IsOpen Then
            CB_RTS.Enabled = True
            CB_DTR.Enabled = True
            LabelCTS.Enabled = True
            LabelDCD.Enabled = True
            LabelDSR.Enabled = True
            LabelRing.Enabled = True


            If CB_DTR.Checked = True Then
                SerialPort.DtrEnable = True
            Else
                SerialPort.DtrEnable = False
            End If

            If CB_RTS.Checked = True Then
                SerialPort.RtsEnable = True
            Else
                SerialPort.RtsEnable = False
            End If
        Else
            CB_RTS.Enabled = False
            CB_DTR.Enabled = False
            LabelCTS.Enabled = False
            LabelDCD.Enabled = False
            LabelDSR.Enabled = False
            LabelRing.Enabled = False
        End If
        ...
        If lstPorts.SelectedIndex <> -1 Then
            Try
                If SerialPort.IsOpen Then
                    ' do nothing
                Else
                    ' if the port is closed.. we may need to guarantee it has a valid name
                ...
                If SerialPort.IsOpen Then
                    ' if the port was open, then we close it and change all the menu items
                    SerialPort.Close()
                ...
                Else
                    ...
                    If SerialPort.IsOpen Then ' this is a dumb question.. we just used an IF statement to see it is not open.
                    ...
                    ' SerialPort.DtrEnable = False
                    ' SerialPort.RtsEnable = False
                    SerialPort.NewLine = vbCrLf ' this may cause problems with a port monitor "echo" console.
                    SerialPort.ReadTimeout = 10000

                    SerialPort.Open()
                    ' after we open the serial port... change all the menu items
            ...
            Catch ex As Exception
                MsgBox(ex.Message) 'not likely to happen unless hardware failure
            End Try
        Else
            MsgBox("Please choose a serial port.. Or a serial port may not be connected to this computer.", vbInformation, "Serial Port")
        End If
    ...
    Private Sub AboutToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AboutToolStripMenuItem.Click
       ...
       & "Watch the data coming into the computer's Com Port.", _
       vbInformation, "About Com Tests Rabbit")
    End Sub
    ...
    Private Sub SerialPort_PinChanged(sender As Object, e As SerialPinChangedEventArgs) Handles SerialPort.PinChanged
        ...
        ' Using a 25-pin Male serial connector
        ' 8 - (Input) DCD - Data Carrier Detect
        ' 3 - (Input) RXD -Receive Data
        ' 2 - (Output) TXD - Transmit Data
        ' 20 - (Output) DTR - Data Terminl Ready
        ' 7 - Signal Ground
        ' 6 - (Input) DSR - Data Set Ready
        ' 4 - (Output) RTS - Request to Send 
        ' 5 - (Input) CTS - Clear To Send 
        ' 22 - (Input) RI - Ring Indicator


        If SerialPort.DsrHolding = True Then
            LabelDSR.ForeColor = Color.Red
        Else
            LabelDSR.ForeColor = Color.Black
        End If

        If SerialPort.CDHolding = True Then
            LabelDCD.ForeColor = Color.Red
        Else
            LabelDCD.ForeColor = Color.Black
        End If

        If SerialPort.CtsHolding = True Then
            LabelCTS.ForeColor = Color.Red
        Else
            LabelCTS.ForeColor = Color.Black
        End If

        If e.EventType = IO.Ports.SerialPinChange.Ring Then ' detect the Ring toggle
            LabelRing.ForeColor = Color.Red
            RingFound = True
        Else
            ' not our concern
        End If

        ' a teaching method... doesn't really do anything
        Select Case True
            Case SerialPort.CDHolding
            Case SerialPort.CtsHolding
            Case SerialPort.DsrHolding
            Case SerialPort.RtsEnable
            Case e.EventType = IO.Ports.SerialPinChange.Ring ' detect the Ringing?
                ' SerialPort
                '  IO.Ports.SerialPinChange.
                ' the ring wire is being triggered .. detects the signal change.
    ...
    Private Sub CB_DTR_CheckedChanged(sender As Object, e As EventArgs) Handles CB_DTR.CheckedChanged
        If CB_DTR.Checked = True Then
            SerialPort.DtrEnable = True
        Else
            SerialPort.DtrEnable = False
    ...
    Private Sub CB_RTS_CheckedChanged(sender As Object, e As EventArgs) Handles CB_RTS.CheckedChanged
        If CB_RTS.Checked = True Then
            SerialPort.RtsEnable = True
        Else
            SerialPort.RtsEnable = False
    ...
    Private Sub TimerSPRing_Tick(sender As Object, e As EventArgs) Handles TimerSPRing.Tick


        If RingFound = True Then
            RingTimer = RingTimer + 1
        End If

        If RingFound = True And RingTimer > 2 Then
            RingFound = False
            RingTimer = 0
            LabelRing.ForeColor = Color.Black
        Else
