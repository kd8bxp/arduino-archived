 ...
 * 
 * Aug 1, 2017 added PIN_UP and PIN_DOWN to dance
 * LeRoy Miller, 2017
 ******************************************/
...
void loop() { /*----( LOOP: RUNS CONSTANTLY )----*/
  ...
  int action = random(0, 8); //Generate a random action for the robot to complete from 0-5 (SPIN_LEFT - DRIVE_REVERSE)
  ...
  else if (action == PIN_UP) {
    analogWrite(SERVO_PIN, 0); //Set the servo to the position held in servoPosition
  }
  else if (action == PIN_DOWN) {
