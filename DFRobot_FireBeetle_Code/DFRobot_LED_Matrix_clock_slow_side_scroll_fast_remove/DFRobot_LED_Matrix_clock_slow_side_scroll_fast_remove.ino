...
void updateDisplay(uint8_t *DIGIT, int number, int loc) {
  ...
  if (loc == 3) {start = 13 + number; offset = 12;} //13
  if (loc == 4) {start = 18 + number; offset = 17;} //17
  for (int i=0; i<9; i++) {
  //display.drawImage(DIGIT,4,8,start,8-i,0);  //slide up
  display.drawImage(DIGIT,4,8,start,i-8,0);
  display.writeScreen();
delay(slideTime);
 }
}
