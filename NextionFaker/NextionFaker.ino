...
/**
 * @name loop()
 * Loops endles through the code. The loop acknowledges 2 modes. if false, we are
 * receiving a connect string at 9600 baud. If a TJC is connected it will be detected
 * while the string "TJC" is replaced with "NX". It is the sent to  to  Editor.
 * IF it is an original "NX", no changes are made and it is sent back asis
 */
void loop()
{
	if (!mode) {
		if (Serial.available() != 0) {					// wait for a character from the NExtion
			...
			Serial.begin(115200);					// and switch to 115200 baud
		}
	} else {										// upload mode
		if (Serial.available() != 0) {				// as long as there are characters from the editor
			if (timer == 0) {						// if the timer is not set, start it
				timer = millis();
			}
			c = Serial.read();						// read them and
			Serial.write(c);						// send them to the Nextion
			//
