...
//July 24 2023- camera code was based in part on code found here:
//https://github.com/sandalhat/UncannyIndex

//July 24 2023 small changes made in prep for using the Useful Sensors Person Sensor
...
#define DEBLINK 2     // Eyelid is currently opening
  ...
  //My variables for receiving and using data from openCV / python
bool useFaceData = true; //false;
...
unsigned long staleTime = 3000;
unsigned long idleTime = 3600000; //1 hour
...
void frame( // Process motion for a single frame of left or right eye
  ...
  // take data from python, constrain to circle
    //int dx, dy;
    //int32_t d;
   delay(15);
 ...
 // for (int i = 0; i < results.num_faces; ++i) {
  const person_sensor_face_t* face = &results.faces[0];
    eyeX = (face->box_left + ((face->box_right)-(face->box_left))/2);  // turns sensor box into x-axis point for eye-contact
    eyeY = (face->box_bottom + ((face->box_top)-(face->box_bottom))/2);  // turns sensor box into y-axis point for eye-contact
 eyeX = abs(map(eyeX, 0, 256, 1023, 256));
 eyeY = abs(map(eyeY, 0, 256, 256, 1024));
  //  dx = (eyeX *2) - 1023;
  // dy = (eyeY *2) - 1023;
    
  //  int  xval = ((dx * -4) + 1023)*1.2;   // translate sensor information into servo info
  //  int yval = ((dy * -4) + 1023)*1.2;
     Serial.print("X,Y "); Serial.print(eyeX); Serial.print(","); Serial.println(eyeY);
  ...
  //}
  //}
    dt = 3000000; //make the time seem long enough that any previous animation is considered complete
  } else {
...
// Blinking
  ...
  drawEye(eyeIndex, iScale, eyeX, eyeY, n, lThreshold); //NOTES: eyeX = 35, eyeY = 35 looks more or less ahead/forward 
  //eyeX = 20, eyeY = 31 looks more or less forward and to right
  //eyeX = 50, eyeY = 30 looks more or less forward and to left
