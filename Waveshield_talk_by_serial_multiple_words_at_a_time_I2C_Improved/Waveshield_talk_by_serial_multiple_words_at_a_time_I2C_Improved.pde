...
void loop() { 
  if (Serial.available() > 0) {
    ...
    while (Serial.available() > 0) {
      ...
      if (transitchar != ' ' && transitchar != ';' && transitchar != ',' && transitchar != '.') { //if char is anything but space or ; or . or , then...
      filename[i] = transitchar; //put char in string
      i++;
      } else if (transitchar == ';') { //if ;
        filename[i] = transitchar;
        Wire.beginTransmission(2); //send the command through i2c to TLC freeduino
      ...
      else if (transitchar == ',') { //play then pause
        add_and_play();
        delay(50);
      }//end else if ,
      else if (transitchar == '.') { //play then big pause
        add_and_play();
        delay(100);
      }//end else if .
