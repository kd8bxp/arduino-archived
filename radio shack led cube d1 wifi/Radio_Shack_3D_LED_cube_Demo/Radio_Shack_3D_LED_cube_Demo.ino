...
void Scan(int totalpage)
{
  for(int a=0;a<totalpage;a++)  
  {
    for(int b=0;b<(table[a][3]>>8);b++)
    {
      ...
      {
        digitalWrite(D2,(table[a][c%3]&0x1));
        digitalWrite(D4,(table[a][c%3]&0x2));   
        digitalWrite(D7,(table[a][c%3]&0x4));
        digitalWrite(D8,(table[a][c%3]&0x8));
        digitalWrite(D9,(table[a][c%3]&0x10));   
        digitalWrite(D10,(table[a][c%3]&0x20)); 
        digitalWrite(D11,(table[a][c%3]&0x40));
        digitalWrite(D12,(table[a][c%3]&0x80));   
        digitalWrite(D13,((table[a][c%3]&0x100)>>1));
        //if(c%3==0)
