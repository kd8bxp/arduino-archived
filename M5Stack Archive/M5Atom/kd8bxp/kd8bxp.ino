...
extern const unsigned char image_kd8bxp[512]; //AtomImageData[2+(4*6+1)*5*3]; //(4文字＊6ドット+1ドット空き)*5ラスタ*3RGB
...
void setup()
{
  ...
  if (M5.IMU.Init() != 0)
    ...
    IMU6886Flag = true;
  //    M5.dis.animation((uint8_t *)image_kd8bxp, 5000, LED_Display::kMoveLeft, 18);
  M5.dis.displaybuff((uint8_t *)image_kd8bxp, 0, 0);
  delay(1000);
}
...
void loop()
{
  int i = 100;
  while (i-- > 0) {
    M5.dis.displaybuff((uint8_t *)image_kd8bxp, i, 0);
