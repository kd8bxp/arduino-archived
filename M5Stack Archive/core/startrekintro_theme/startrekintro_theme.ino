...
void setup() {
  ...
  ledcSetup(SPK, spkFreq, resolution);
  ledcAttachPin(SPEAKERPIN, SPK);
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    beep(melody[thisNote], noteDuration); // * 0.9);
...
{   
      uint16_t delay_interval = ((uint32_t)1000000 / noteFrequency);
      ledcWriteTone(SPK, noteFrequency);
