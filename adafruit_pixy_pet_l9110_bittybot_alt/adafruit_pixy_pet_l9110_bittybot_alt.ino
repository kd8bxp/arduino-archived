...
digitalWrite( MotorRightPWM, LOW);
    ...
    void FollowBlock(int trackedBlock)
    {
     ...
     digitalWrite(MotorLeftDIR, HIGH ); // direction = forward
        analogWrite(MotorLeftPWM, leftSpeed); 
        digitalWrite(MotorRightDIR, HIGH ); // direction = forward
        analogWrite(MotorRightPWM, rightSpeed); //255-(speedPWM-speedPWMR)
    }
    ...
    void ScanForBlocks()
    {
    	if (millis() - lastMove > 20)
    	{
    		...
    		if ((panLoop.m_pos >= RCS_MAX_POS)||(panLoop.m_pos <= RCS_MIN_POS))
    		{
    			...
    			{
    						 digitalWrite(MotorLeftDIR, HIGH ); // direction = forward
        analogWrite(MotorLeftPWM, 250); 
        digitalWrite(MotorRightDIR, LOW ); // direction = forward
        analogWrite(MotorRightPWM, 250); //255-(speedPWM-speedPWMR)
    				
    			}
    			else
    			{
    				 digitalWrite(MotorLeftDIR, LOW ); // direction = forward
        analogWrite(MotorLeftPWM, 180); 
        digitalWrite(MotorRightDIR, HIGH ); // direction = forward
        analogWrite(MotorRightPWM, 180); //255-(speedPWM-speedPWMR)
    			}
