...
unsigned long millis(void)
{
	...
	cli();
	unsigned long result = seconds_count*1000UL + CountSec;
	SREG = oldSREG;
...
uint64_t micros_huge(void) {
	...
	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	uint64_t result = ((uint64_t)seconds_count)*1000000UL + CountSec*1000UL + (TCD0.CNT>>2);
	SREG = oldSREG;
...
unsigned long micros(void) {
	...
	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	//unsigned long result = seconds_count*1000000UL + millis_count*1000 + TCF1.CNT*1000UL + (TCD0.CNT>>2);
	unsigned long result = millis_count*1000 + (TCD0.CNT>>2); //TIAN
	SREG = oldSREG;
