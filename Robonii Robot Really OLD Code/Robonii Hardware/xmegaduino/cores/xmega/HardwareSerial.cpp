...
#define INTVECTORS_APP asm("push r30"      "\n\t"	\
          "push r31"     "\n\t"	\
          "ldi r31,0xD8"     "\n\t"	\
    			"ldi	r30, 0x00"     "\n\t"	\
          "out	0x34, r31"     "\n\t"	\
...
// Inidicate XMEGA power to USB chip
  PORTF.PIN4CTRL = PORT_SRLEN_bm | PORT_OPC_PULLUP_gc;
  
 /* 
 //Kry verseker data op pin
   char c = PORTF.IN & _in_bm;
  while (1)
  {
	if (c !=(PORTF.IN& _in_bm))
	{
	PORTK.DIRSET = 0x02; //TIAN
	//PORTK.DIRSET = _in_bm; //TIAN
	//PORTK.DIRCLR = _in_bm; //TIAN
	//PORTK.DIRCLR = 0x0f;;
	c = PORTF.IN& _in_bm;
	int i = 20000;
	while (i--);
  ...
  */
  ...
  _usart->CTRLA = 0x10; //TIAN // Enable only RX int aty low

  if (_usart->CTRLA != 0x10)
  {
  PORTK.DIRSET = 0x02; //TIAN
  while(1);
  }
