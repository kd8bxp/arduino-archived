...
{
    ADC_t* adc;
#if defined(Robonii_h)
//ROBONII only uses first 4 of Channel A, and then ChannelB
		   if ( pin < 4 ) {
        adc = &ADCA;
    } else if ( pin < 8 ) {
        adc = &ADCB;
        pin -= 4; //TIAN: To remove inorder to use JTAG A2D. NMioet   
	...
	//adc->REFCTRL     = 0;//TIAN Fix to 1V internal...
...
//TIAN +
   (uint16_t)&TCF0.CCABUF,
    (uint16_t)&TCF0.CCBBUF,
    (uint16_t)&TCF0.CCCBUF,
    (uint16_t)&TCF0.CCDBUF,
    (uint16_t)&TCF1.CCABUF,
    (uint16_t)&TCF1.CCBBUF,
	//TIAN +
