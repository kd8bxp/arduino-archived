...
void KeyPadLoop(){
  if (Serial.available()) {
    ...
    if (r == '5') { ClearKeys();  but_UP=true; }  //else but_UP=false;
    if (r == '2') { ClearKeys();  but_DOWN=true; }  //else but_DOWN=false;
    if (r == '1') { ClearKeys();  but_LEFT=true; }  // else but_LEFT=false;
    if (r == '3') { ClearKeys();  but_RIGHT=true; }  //else but_RIGHT=false;
  }
	M5.update();
	uint8_t state_x = M5.JOY_X.isAxisPressed();
	uint8_t state_y = M5.JOY_Y.isAxisPressed();
	if (state_x) {
	  ClearKeys();
	  if (state_x == 2)			// joy x left -> down
		  but_DOWN = true;
	  else if (state_x == 1)	// joy x right -> up
		  but_UP = true;
	}

	if (state_y) {
	  ClearKeys();
	  if (state_y == 2)			//joy y up -> left
		  but_LEFT = true;
	  else if (state_y == 1)	//joy y down -> right
		  but_RIGHT = true;
	}
	if (M5.BtnA.wasPressed()) { ClearKeys(); but_A = true; delay(300);}
	if (M5.BtnB.wasPressed()) { ClearKeys(); but_B = true; delay(300); }  else but_B=false;
}
