...
void XModem::sendFile(File dataFile, char *fileName)
{
	...
	// ******* ADDED Block for CRC error checking **********
	byte block = 0;
	...
	while (!finished)
	{
		...
		do
		{
			...
			if (oldChecksum){
				port->write((char)255-checksumBuf);
				Serial.println("oldie method");

			} else
			{
				//port->write((char) (crcBuf >>8)); 	// M100 and M102 method
				//port->write((char) (crcBuf & 0xFF)); // And this line

				port->write(block);						// NEC PC-8401 method

				Serial.println("newbie method CRC");
