...
poly_div(const Poly *p, const Poly *q, Poly *newp) {
    ...
    __size_t sep = p->length-(q->length-1);
...
public:
     ...
     * @return RESULT_SUCCESS if successfull, error code otherwise */
     int DecodeBlock(const void* src, const void* ecc, void* dst, uint8_t* erase_pos = NULL, __size_t erase_count = 0) {
        assert(msg_length + ecc_length < 256);
     ...
     * @param erase_count  - count of known errors
     * @return RESULT_SUCCESS if successfull, error code otherwise */
     int Decode(const void* src, void* dst, uint8_t* erase_pos = NULL, __size_t erase_count = 0) {
         const uint8_t *src_ptr = (const uint8_t*) src;
...
private:
    ...
    bool FindErrorLocator(const Poly *synd, Poly *erase_loc = NULL, __size_t erase_count = 0) {
    ...
    bool FindErrors(const Poly *error_loc, __size_t msg_in_size) {
        Poly *err = &polynoms[ID_ERRORS];
    ...
    void CalcForneySyndromes(const Poly *synd, const Poly *erasures_pos, __size_t msg_in_size) {
