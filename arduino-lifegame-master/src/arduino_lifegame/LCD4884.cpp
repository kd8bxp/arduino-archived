...
void LCD4884::SetInterval(int Time)
{
	flash_interval = Time;
}
...
void LCD4884::blink()
{
	...
	static boolean backlight_State = false;
	if(millis() - timer >= flash_interval){
		if(backlight_State)	backlight(OFF),backlight_State = false;
		else	backlight(ON),backlight_State = true;
		timer = millis();
	}
}
