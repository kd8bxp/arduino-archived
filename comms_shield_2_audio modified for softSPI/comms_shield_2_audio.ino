...
int cs=10;          // this is the chip-select pin for mySPI
...
void setup()  
{
  ...
  //mySPI.setClockDivider(SPI_CLOCK_DIV32); // low frequency mySPI
  ...
  //mySPI.setBitOrder(MSBFIRST);  // OTP requires MSB first
  //mySPI.setDataMode(SPI_MODE0);  // Use MODE0, as all other modes to not work
  value2 = 0x00; // Start at voice group 0
  delay(500);   // One second delay
  // ramp up Little Buddy Talker chip
  digitalWrite(cs,LOW);
  
  SSPI.transfer(value3);
  SSPI.transfer(0x00);
...
void loop() {
  readout();     // mySPI command transfer function.  Whenever you call this function, it calls a word to be played. LEDs will blink, LCD written to, and backlight turned on/off
...
void readout()
{
  ...
  SSPI.transfer(value1);   // Send fixed value1 integer to LBT
  SSPI.transfer(value2);   // Send word address and play
